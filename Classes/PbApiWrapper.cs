...
internal class PbApiWrapper
{
    ...
    private readonly Func<Sandbox.ModAPI.Ingame.IMyTerminalBlock, VRage.Game.ModAPI.Ingame.IMyEntity, bool, bool>
        _entityBypass;
    ...
    public void SetActiveShield(Sandbox.ModAPI.Ingame.IMyTerminalBlock block) =>
        _block = block; // AutoSet to TapiFrontend(block) if shield exists on grid.

    public PbApiWrapper(Sandbox.ModAPI.Ingame.IMyTerminalBlock block)
    {
        _block = block;
        var delegates = _block.GetProperty("DefenseSystemsPbAPI")?.As<IReadOnlyDictionary<string, Delegate>>()
            .GetValue(_block);
        if (delegates == null) return;

        _rayIntersectShield =
            (Func<Sandbox.ModAPI.Ingame.IMyTerminalBlock, RayD, Vector3D?>)delegates["RayIntersectShield"];
        _lineIntersectShield =
            (Func<Sandbox.ModAPI.Ingame.IMyTerminalBlock, LineD, Vector3D?>)delegates["LineIntersectShield"];
        ...
        _entityBypass =
            (Func<Sandbox.ModAPI.Ingame.IMyTerminalBlock, VRage.Game.ModAPI.Ingame.IMyEntity, bool, bool>)delegates[
                "EntityBypass"];
        ...
        _getShieldBlock =
            (Func<VRage.Game.ModAPI.Ingame.IMyEntity, Sandbox.ModAPI.Ingame.IMyTerminalBlock>)delegates[
                "GetShieldBlock"];
        ...
        _getDistanceToShield =
            (Func<Sandbox.ModAPI.Ingame.IMyTerminalBlock, Vector3D, double>)delegates["GetDistanceToShield"];
        _getClosestShieldPoint =
            (Func<Sandbox.ModAPI.Ingame.IMyTerminalBlock, Vector3D, Vector3D?>)delegates["GetClosestShieldPoint"];
    ...
    public bool EntityBypass(VRage.Game.ModAPI.Ingame.IMyEntity entity, bool remove = false) =>
        _entityBypass?.Invoke(_block, entity, remove) ?? false;

    public bool EntityBypassPb(long entity, bool remove = false) =>
        _entityBypassPb?.Invoke(_block, entity, remove) ?? false;
    ...
    public bool ProtectedByShield(VRage.Game.ModAPI.Ingame.IMyEntity entity) =>
        _protectedByShield?.Invoke(entity) ?? false;

    public Sandbox.ModAPI.Ingame.IMyTerminalBlock GetShieldBlock(VRage.Game.ModAPI.Ingame.IMyEntity entity) =>
        _getShieldBlock?.Invoke(entity) ?? null;

    public bool IsShieldBlock() => _isShieldBlock?.Invoke(_block) ?? false;

    public Sandbox.ModAPI.Ingame.IMyTerminalBlock GetClosestShield(Vector3D pos) =>
        _getClosestShield?.Invoke(pos) ?? null;
